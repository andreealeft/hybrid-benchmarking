@echo off
rem Double-click this once.  It installs the tool into an environment of its
rem own and puts an icon on the Desktop that opens it.  Nothing is typed, and
rem nothing else on the computer is touched: removing the tool is removing one
rem folder.
title Hybrid benchmarking

set HOME_DIR=%LOCALAPPDATA%\hybrid-benchmarking
set VENV=%HOME_DIR%\venv
set SOURCE=https://github.com/andreealeft/hybrid-benchmarking/archive/refs/heads/main.zip

echo.
echo   Hybrid benchmarking
echo   ===================
echo.

rem ---------------------------------------------------------------- Python
set PY=
where py >nul 2>nul && set PY=py
if "%PY%"=="" (where python >nul 2>nul && set PY=python)
if "%PY%"=="" (
    echo   This needs Python, which is not on this computer yet.
    echo   Opening the download page now.
    echo.
    echo   Install it, tick "Add Python to PATH" in the installer,
    echo   then double-click this file again.
    start "" "https://www.python.org/downloads/windows/"
    echo.
    pause
    exit /b 0
)

rem ---------------------------------------------------------------- the tool
echo   Installing. This takes a minute.
echo.
if not exist "%VENV%\Scripts\python.exe" (
    %PY% -m venv "%VENV%"
    if errorlevel 1 (
        echo   The environment could not be created.
        pause
        exit /b 1
    )
)
"%VENV%\Scripts\python.exe" -m pip install --upgrade --quiet pip
"%VENV%\Scripts\python.exe" -m pip install --upgrade --quiet "%SOURCE%"
if errorlevel 1 (
    echo   The install did not finish. The message above says why.
    pause
    exit /b 1
)

rem ---------------------------------------------------------------- the icon
rem pythonw runs without a console window, so the icon behaves like an app
rem rather than opening a black box of text.
powershell -NoProfile -Command ^
  "$s = (New-Object -ComObject WScript.Shell).CreateShortcut([Environment]::GetFolderPath('Desktop') + '\Hybrid benchmarking.lnk');" ^
  "$s.TargetPath = '%VENV%\Scripts\pythonw.exe';" ^
  "$s.Arguments  = '-m hybrid_benchmarking.cli open';" ^
  "$s.WorkingDirectory = '%HOME_DIR%';" ^
  "$s.Description = 'Resource estimates for quantum algorithms';" ^
  "$s.Save()"

echo.
echo   Done.
echo.
echo   There is now an icon on your Desktop called Hybrid benchmarking.
echo   Double-click it whenever you want the tool: it opens in your browser.
echo   It keeps itself up to date. It stops when you restart the computer,
echo   or from the Task Manager, where it is called pythonw.
echo.
pause
